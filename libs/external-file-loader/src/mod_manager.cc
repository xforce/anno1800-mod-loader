        ...
        mod.ForEachFile([this](const fs::path& game_path, const fs::path& file_path)
            {
                ...
                else
                {
                    if (IsPythonStartScript(file_path))
                    {
                        ...
                        std::string startScript = "console.startScript('mods\\"
                    ...
                    {
                        auto hFile = CreateFileW(file_path.c_str(), GENERIC_READ, FILE_SHARE_READ,
...
void ModManager::WriteCacheInfo(const fs::path& game_path)
{
    ...
    for (auto file : fs::directory_iterator(cache_directory / game_path))
    {
        ...
        auto it = std::find_if(begin(modded_file_cache_info_[game_path]),
            end(modded_file_cache_info_[game_path]), [file_name](const auto& x)
            {
                return file_name == x.layer_file;
            });
        //
...
std::optional<std::string> ModManager::CheckCacheLayer(
    const fs::path& game_path, const std::string& input_hash, const std::string& patch_hash)
{
    if (input_hash.empty())
    {
        return {};
...
std::string ModManager::PushCacheLayer(const fs::path& game_path,
    const std::string& last_valid_cache, const std::string& patch_file_hash, const std::string& buf,
    const std::string& mod_name)
{
    ...
    size_t const cSize = ZSTD_compress(
        CompressedBuffer.data(), CompressedBuffer.size(), buf.data(), buf.size(), 1);
            ...
            for (auto&& modded_file : modded_patchable_files_)
            {
                ...
                for (auto&& on_disk_file : on_disk_files)
                {
                    ...
                    else
                    {
                        ...
                        last_valid_cache = PushCacheLayer(game_path, last_valid_cache,
                            patch_file_hash, buf, on_disk_file.string());
