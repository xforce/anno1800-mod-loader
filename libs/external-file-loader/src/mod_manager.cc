...
void ModManager::CollectPatchableFiles() {
  for (const auto& mod : mods) {
    mod.ForEachFile([this](const fs::path& game_path,
      ...
      if (IsPatchableFile(game_path)) {
        ...
        if (IsPythonStartScript(file_path)) {
          ...
          auto hFile =
              CreateFileW(file_path.c_str(), GENERIC_READ, FILE_SHARE_READ,
      ...
      try {
        ...
        if (PATCH_OP_VERSION == patch_op_version) {
          ...
          for (auto&& layer : layer_order) {
            cache_layers.emplace_back(
                data["layers"].at(layer).get<CacheLayer>());
          }
...
void ModManager::WriteCacheInfo(const fs::path& game_path) {
  ...
  for (auto file : fs::directory_iterator(cache_directory / game_path)) {
    ...
    auto it = std::find_if(
        begin(modded_file_cache_info_[game_path]),
...
std::optional<std::string> ModManager::CheckCacheLayer(
...
std::string ModManager::ReadCacheLayer(const fs::path& game_path,
                                       const std::string& input_hash) {
  const auto cache_directory = ModManager::GetCacheDirectory();
  ...
  for (auto&& cache : modded_file_cache_info_[game_path]) {
    if (cache.output_hash == input_hash) {
      ...
      if (file.read(buffer.data(), size)) {
        ...
        output.resize(rSize);
        size_t dSize = ZSTD_decompress(output.data(), output.size(),
...
std::string ModManager::PushCacheLayer(const fs::path& game_path,
  ...
  auto it =
      find_if(begin(cache), end(cache), [&last_valid_cache](const auto& x) {
  ...
  fs::create_directories(cache_directory / game_path);
  std::ofstream ofs((cache_directory / game_path / layer.layer_file),
                    std::ofstream::binary);
...
void ModManager::GameFilesReady() {
  ...
  patching_file_thread = std::thread([this]() {
    ...
    for (auto&& modded_file : modded_patchable_files_) {
      ...
      if (game_file.empty()) {
        spdlog::error("Failed to get original game file {}",
                      game_path.string());
        continue;
      }
      ...
      for (auto&& on_disk_file : on_disk_files) {
        ...
        if (output_hash) {
          ...
          if (!game_xml) {
            ...
            game_xml = xmlReadMemory(cache_data.data(), cache_data.size(), "",
                                     "UTF-8", XML_PARSE_RECOVER);
          }

          // Cache miss
          auto operations =
              XmlOperation::GetXmlOperationsFromFile(on_disk_file);
          ...
          buf = buf.substr(buf.find("<MEOW_XML_SUCKS>") +
          ...
          last_valid_cache =
              PushCacheLayer(game_path, last_valid_cache, patch_file_hash, buf,
                             on_disk_file.string());
...
const ModManager::File& ModManager::GetModdedFileInfo(
  ...
  throw std::logic_error(
      "GetModdedFileInfo shouldn't be called on a file that is not modded");
}
...
fs::path ModManager::GetModsDirectory() {
  ...
  if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
    ...
    try {
      mods_directory =
          fs::canonical(dll_file.parent_path() / ".." / ".." / "mods");
